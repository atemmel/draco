...
pub const Editor = struct {
    ...
    pub fn resize(self: *Editor, width: f32, height: f32) void {
        self.width = width;
        self.height = height;
    }
    ...
    fn reindexVirtualLines(self: *Editor) !void {
    ...
    pub fn insert(self: *Editor, what: []const u8) void {
    ...
    fn byte(self: *Editor, idx: usize) u8 {
    ...
    fn bytesUntilNearestCodepointRight(self: *Editor) usize {
    ...
    fn codepointsLeftOfCursor(self: *Editor) usize {
        const pos = self.virtualCursorPos();
    ...
    fn byteOfNthCodepointOfVirtualLine(self: *Editor, line: VirtualLine, n: usize) usize {
    ...
    pub fn beginningOfLine(self: *Editor) void {
        const pos = self.virtualCursorPos();
    ...
    pub fn endOfLine(self: *Editor) void {
        const pos = self.virtualCursorPos();
        const line = self.allRealLines()[pos.virtual_row];
    ...
    pub fn removeRightCursor(self: *Editor) void {
        if (self.cursor < self.buffer.items.len) {
            const n = self.bytesUntilNearestCodepointRight();
            for (0..n) |_| {
    ...
    pub fn removeLeftCursor(self: *Editor) void {
        if (self.cursor > 0) {
            const n = self.bytesUntilNearestCodepointLeft();
            for (0..n) |_| {
    ...
    pub fn allVirtualLines(self: *Editor) []const VirtualLine {
    ...
    pub fn cursorDrawData(self: *Editor) struct {
        virtual_row: f32,
